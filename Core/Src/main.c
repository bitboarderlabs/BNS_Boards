...
/* USER CODE BEGIN PFP */
//void Fix_Clock_For_Baby_Board(void);  //req'd for baby boards since they have 8MHx HSE xtals instead of 16MHz
void Baby_SystemClock_Config(void);
void Normal_SystemClock_Config(void);
int newMain(void);	//used as the actual "main" since we're customizing the clock setup.
/* USER CODE END PFP */
...
int newMain(void){
	//This sub is used as the replacement main() so that we can customize the clock init.

	// --- *** IMPORTANT *** --- //
	// This must be located in the "USER CODE BEGIN Header" section of stm32f4xx_hal_conf_template.h
